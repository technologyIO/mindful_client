...
export default function OtpForm({ otpSucceed, setPrefilledData, prefilledData , closeModal}) {
  ...
  const handleSendOtp = async () => {
    ...
    setMessage("")
    try {
      const res = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}otp/send-otp`, { phone: phone })
      if (res.data.success) {
        setStep(2)
  ...
  const handleVerifyOtp = async () => {
    ...
    setMessage("")
    try {
      const res = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}otp/verify-otp`, { phone, otp })
      if (res.data.success) {
        setStep(3)
        setPrefilledData({ ...prefilledData, phone })
      } else {
        ...
        <div className="mt-8 flex justify-center space-x-2">
          ...
          <div
            className={`w-3 h-3 rounded-full transition-all duration-300 ${
              step >= 2 ? (step === 2 ? "bg-blue-500" : "bg-green-500") : "bg-gray-200"
            }`}
          />
          <div
